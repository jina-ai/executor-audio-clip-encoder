...
class LinearFBSP(torch.nn.Module):
    def __init__(self, out_features: int, bias: bool = True, normalized: bool = False):
        ...
        self.m = torch.nn.Parameter(torch.zeros(self.out_features, dtype=default_dtype))
        self.fb = torch.nn.Parameter(torch.ones(self.out_features, dtype=default_dtype))
        self.fc = torch.nn.Parameter(
            torch.arange(self.out_features, dtype=default_dtype)
        )
